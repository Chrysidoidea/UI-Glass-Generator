...
export const GlassGeneratorContext = createContext<
  ContextValueType | undefined
>(undefined);

export const GlassGeneratorProvider: React.FC<GlassGeneratorProviderType> = ({
  children,
}) => {
  ...
  const bGColor = `rgba(${hexToRGB(color).r},${hexToRGB(color).g},${
    hexToRGB(color).b
  },${transparensyConverter(transparency)})`;
  const blurConverted = `blur(${blurConverter(blur)}px)`;
  const borderConverted = `1px solid rgba(255, 255, 255, ${transparensyConverter(
    outline
  )})`;
...
export const useGlassGeneratorContext = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useGlassGeneratorContext have to use with GlassGeneratorProvider"
    );
  }
